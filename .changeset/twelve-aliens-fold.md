...
使用 Windows 内置 PowerShell 与进程 API 探测已有 HBuilderX 实例，避免 WMIC 缺失时误判为未启动并重复打开 IDE。通过带完整性标识的 Base64 路径列表保留中文、逗号和共享路径，探测失败时明确报错。
...
子进程 stdout/stderr 分别按流解码 UTF-8，避免中文项目名和错误文本被数据块边界截断成乱码。
