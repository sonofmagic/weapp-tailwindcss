import type { EvidenceSummary } from '../config'
import { Img, interpolate, staticFile, useCurrentFrame } from 'remotion'
import { eyebrowStyle, SceneShell } from '../components/SceneShell'
import { COLORS } from '../config'

export function EvidenceScene({ evidence }: { evidence: EvidenceSummary }) {
  ...
  return (
    <SceneShell style={{ display: 'grid', gridTemplateColumns: '1.08fr 0.92fr', gap: 72, alignItems: 'center' }}>
      <div>
        <div style={eyebrowStyle}>兼容性实验室</div>
      ...
      <div style={{ position: 'relative', height: 710, overflow: 'hidden', border: `1px solid ${COLORS.border}`, borderRadius: 8, background: COLORS.surface, opacity: imageOpacity }}>
        ...
        <div style={{ position: 'absolute', left: 20, right: 20, bottom: 20, padding: '16px 18px', borderRadius: 8, background: 'rgba(9, 13, 14, 0.92)', color: '#cbd7d4', fontSize: 17 }}>支持项与限制项都来自已提交的双端报告</div>
      </div>
    </SceneShell>
  )
}
