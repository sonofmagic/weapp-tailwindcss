...
describe('Expo example manifest', () => {
  it('uses an explicit entrypoint for pnpm workspace layouts', () => {
    const packageJson = JSON.parse(fs.readFileSync(path.resolve(exampleRoot, 'package.json'), 'utf8')) as { main: string }
    expect(packageJson.main).toBe('src/index.ts')
    expect(fs.existsSync(path.resolve(exampleRoot, packageJson.main))).toBe(true)
  })
