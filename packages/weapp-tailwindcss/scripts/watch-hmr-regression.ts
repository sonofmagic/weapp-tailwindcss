...
interface SameClassLiteralHmrMetrics {
  markerBefore: string
  markerAfter: string
  classLiteral: string
  escapedClasses: string[]
...
async function runClassMutation(
  ...
  if (mutationKind === 'script') {
    const seed = `${Date.now().toString().slice(-6)}${Math.floor(Math.random() * 1000).toString().padStart(3, '0')}`
    const markerBefore = `tw-watch-${watchCase.name}-${mutationKind}-same-before-${seed}`
    const markerAfter = `tw-watch-${watchCase.name}-${mutationKind}-same-after-${seed}`
    ...
    if (
      ...
      throw new Error(`[${watchCase.label}] failed to build same-class literal mutation source for ${mutationKind}`)
    }
    ...
    process.stdout.write(
      `[watch-hmr] ${watchCase.label} mutation=${mutationKind} same-class-literal passed (hotUpdate=${hotUpdateEffectiveMs}ms, rollback=${rollbackEffectiveMs}ms)\n`,
    )
