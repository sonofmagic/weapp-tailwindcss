...
- command: `pnpm --filter weapp-tailwindcss exec vitest bench test/core.bench.ts test/bundlers.bench.ts --config ./vitest.config.ts --reporter=default --reporter=json --outputFile.json=benchmark/bench-report.json`
- vitest benchmark output: `benchmark/bench-report.json`，由 Vitest 5 顶层 JSON reporter 生成。
- options: webpack time=150ms iterations=3 warmupIterations=1; vite time=150ms iterations=3; gulp time=120ms iterations=3; core benches use time-based runs with 10 minimum iterations and 1 warmup iteration.
