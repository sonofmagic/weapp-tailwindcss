...
describe('bundlers/webpack WeappTailwindcss', () => {
  ...
  it('does not regenerate taro main wxss after the source css entry was generated by webpack loaders', async () => {
    ...
    let loaderHandler: ((loaderContext: any, module: LoaderModule) => void) | undefined
    let assetStore: Record<string, string> = {
      'app.wxss': `.${generatedClass}{font-size:40px}`,
    }
    const compilation = {
      compiler: { outputPath: path.resolve(process.cwd(), 'dist') },
      chunks: [{ id: 'app', hash: 'hash-generated', files: ['app.wxss'] }],
  ...
  it('generates Taro webpack v4 css from loader registered source css when final wxss has no Tailwind directives', async () => {
    ...
    try {
      ...
      loaderHandler?.({}, sourceCssModule)
      const classSetLoaderEntry = sourceCssModule.loaders.find(entry => entry.loader === currentContext.runtimeLoaderPath)
      const loaderRuntime = getWebpackLoaderRuntime(classSetLoaderEntry?.options?.weappTailwindcssRuntimeKey)
      loaderRuntime?.classSet?.registerCssSourceFile?.({
        file: sourceCssFile,
        css: sourceCss,
  ...
  it('refreshes webpack registered source css options after style hot updates', async () => {
    ...
    try {
      ...
      expect(currentContext.styleHandler.mock.calls.at(-1)?.[0]).toBe(secondSourceCss)
      expect(currentContext.styleHandler.mock.calls.at(-1)?.[1]?.sourceOptions?.sourceCss).toContain('@apply font-bold')
