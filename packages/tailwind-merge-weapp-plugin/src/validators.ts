...
const arbitraryValueRegex = new RegExp(`^${SimpleMappingChars2String['[']}(?:([a-z-]+):)?(.+)${SimpleMappingChars2String[']']}$`, 'i')
...
const numberRegex = new RegExp(`^(?:\\d+${SimpleMappingChars2String['.']}\\d+|\\d+)$`)

export function isNumber(value: string) {
  return numberRegex.test(value)
  // return (Boolean(value) && !Number.isNaN(Number(value))) || (/^(?:\d+d\d+|\d+)$/.test(value))
}
