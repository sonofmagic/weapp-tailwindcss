...
- If an install/run command is required, refer to the dev script in the corresponding demo project's `package.json`; do not use a repository-root workspace filter command.
```
...
- Run the React or Vue Demo: enter the corresponding demo directory and run its own dev script.
- Build the documentation (optional): repository maintainers should follow the contribution guide; ordinary users do not need to build the documentation site.
