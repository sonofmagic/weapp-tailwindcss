...
Starting with `weapp-tailwindcss@5.2.0`, Node.js `>=22.12.0` is required. Node.js 22 supports loading ESM from CommonJS by default starting from this version, which is also the engines requirement of the current warehouse and core packages. When the Node version is not satisfactory, it is not recommended to use `--ignore-engines` hard installation, and subsequent build errors will be more difficult to troubleshoot.
