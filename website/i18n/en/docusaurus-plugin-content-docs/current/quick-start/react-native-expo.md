...
Run the development or build command provided by your own Expo project's `package.json`, then confirm:

- Metro loads the CSS entry and static `className` values hit the generated StyleSheet lookup.
- Warnings in `manifest.warnings` are reviewed, and unknown CSS is not mistaken for a React Native style.
- Expo Web is only a smoke test; Android and iOS simulators or devices render layout, color scheme, and platform variants correctly.
- After CSS or source changes, HMR refreshes the manifest and dynamic classes still resolve only generated candidates.
