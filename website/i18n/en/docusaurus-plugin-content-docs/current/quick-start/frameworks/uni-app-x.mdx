...
See the [uni-app x configuration reference](../../config/uni-app-x) for option defaults and local-style matching rules.
...
HBuilderX depends on the local installation. In your own project, run the Web, mini-program, Android, iOS, and HarmonyOS targets separately and inspect the real artifacts and runtime results.
...
HBuilderX remains the source of truth for uni-app x running and building. For command-line automation, use the HBuilderX CLI documented by DCloud; validate each target's output directory, CSS suffix, and runtime behavior separately.
