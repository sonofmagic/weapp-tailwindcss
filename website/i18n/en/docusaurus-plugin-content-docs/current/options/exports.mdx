...
description: >-
  weapp-tailwindcss provides both ESM and CommonJS entrances, and has multiple built-in secondary exports to adapt to
  different combinations of webpack, Vite, Gulp, Tailwind macros, etc. The following is a description based on usage
  classification.
keywords:
  - Configuration items
  - Plug-in parameters
...
| `weapp-tailwindcss/vite/web` | Generic Vite Web entry with a fixed Web target and no mini-program framework detection | `import tailwindcss from 'weapp-tailwindcss/vite/web'`
...
Standard Vite Web or SSR projects can use the dedicated entry without configuring `appType`, `generator.target`, or `tailwindcssBasedir`:
...
This entry owns Tailwind Web CSS generation and the required HMR path only. Mini-program template/JS transforms, subpackage styles, and platform CSS adapters stay out of the Web production hot path. Pass `optimize: false` or `{ minify: false }` to disable generated CSS optimization.
