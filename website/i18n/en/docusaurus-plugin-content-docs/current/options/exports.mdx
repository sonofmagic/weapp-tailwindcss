...
| `weapp-tailwindcss/core` | Exposes framework-level `createCompiler` and the compatible `createContext` API for generation, snapshots, and transforms | `import { createCompiler } from 'weapp-tailwindcss/core'`
...
Standard Vite Web or SSR projects use the main `weapp-tailwindcss/vite` entry directly. Unmarked Generic Vite projects default to the Web target after config resolution; explicit `appType`, `generator.target`, `platform`, and `cssEntries` always take precedence. Historical Generic mini-program projects should set `generator.target: 'weapp'` or `platform` explicitly.
