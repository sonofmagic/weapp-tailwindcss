...
The CLI directly reuses the Tailwind v4 generator, design system, and source graph provided by `weapp-tailwindcss`. It does not install or invoke `@tailwindcss/cli`. Watch mode consistently uses cross-platform polling and does not depend on `@parcel/watcher`.
...
`@weapp-tailwindcss/cli` provides the command layer, `weapp-tailwindcss` provides generation and CSS transformation, and `tailwindcss` provides the Tailwind CSS v4 version used by the project.
