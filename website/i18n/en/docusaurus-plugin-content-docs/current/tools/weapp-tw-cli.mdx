...
See [Quick start: Install dependencies](/docs/quick-start/install) for installation. `@weapp-tailwindcss/cli` provides the command layer, `weapp-tailwindcss` provides generation and CSS transformation, and `tailwindcss` provides the Tailwind CSS v4 version used by the project.
