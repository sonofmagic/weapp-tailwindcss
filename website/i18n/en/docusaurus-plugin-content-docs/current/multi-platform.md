...
The message distinguishes the theme setting from remaining runtime `calc`, or output with no related expression detected. Successful `cssCalc` static evaluation still receives the configuration advisory; `@theme inline` alone does not guarantee static output. Failed diagnosis is skipped, and absence of a warning does not establish safety. See [Build-time advisory warning](./issues/spacing-rpx.md#build-time-advisory-warning) for scope and device verification limits.
...
`--spacing: 1rpx` can generate utilities, but runtime `calc` can produce a different size from a direct `rpx` length. Native WXSS checks also reproduced a discrepancy with a `3rpx` base. Prefer `px` for fixed pixel dimensions, or a final static `rpx` length for scaling with the window width. If retaining a runtime base, consider larger or even `rpx` values, but even values do not guarantee accuracy and still require device testing. The current mainline fixes the missing fixed-theme context in the mini-program deferred and incremental paths: with explicit `cssCalc: ['--spacing']` and a build-time-resolvable variable, evaluation happens before theme scoping, so `--spacing: 1rpx` can produce `32rpx` and `4rpx` for `w-32` and `p-4`. Runtime overrides, unresolved variables, or variables that are not selected can still retain the expression. See [WeChat --spacing and rpx calculation limits](./issues/spacing-rpx.md) for comparisons, version scope, and guidance.
:::
...
`cssCalc: false` does not generate precomputed fallbacks. `cssCalc: ['--spacing']` retains static results and removes duplicate variable declarations after successful precomputation. Rebuild the target, inspect the final properties in WXSS/CSS, and compare rendered sizes with direct lengths on the target device. If `calc(var(--spacing) * N)` remains, check for runtime overrides, unresolved theme context, or a later plugin that recreated the expression; do not treat static evaluation as a fix for WeChat's runtime algorithm.
