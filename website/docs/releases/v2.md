---
title: 2.x 版本新特性
description: 这个版本新增了 WeappTailwindcss
keywords:
  - 2.x
  - 版本新特性
  - releases
...
`WeappTailwindcss` 是一个核心插件，所有使用 `webpack` 进行打包的框架都可以使用它，只需要传入 `appType` 配置项: `uni-app`/`taro`/`rax`/`remax`/`mpx` 等等，如果不传的话，插件会去猜测公共的样式文件位置，并进行转化(有可能不准确)。
...
`WeappTailwindcss` 为 `vite` 专用插件，配置项和使用方式也是和上面一致的。
