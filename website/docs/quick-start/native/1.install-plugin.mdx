...
Tailwind CSS 4 的扫描范围写在 CSS 入口的 `@source` 里。生成模式下不要再注册 Tailwind 官方生成插件，也不需要执行 `weapp-tw patch`。当前文档仅维护 Tailwind CSS 4 接入说明。

Tailwind CSS 4 项目推荐显式配置 `cssEntries`。`cssEntries` 只负责让 `WeappTailwindcss` 稳定读取入口 CSS，入口 CSS 仍然要通过项目入口实际引入；如果有多个 Tailwind CSS 入口，就都写进数组：
