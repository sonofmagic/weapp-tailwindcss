...
从 `weapp-tailwindcss@5.2.0` 开始，需要 Node.js `>=22.12.0`。Node.js 22 从该版本开始默认支持从 CommonJS 加载 ESM，这也是当前仓库与核心包的 engines 要求。Node 版本不满足时，不建议用 `--ignore-engines` 硬装，后续构建错误会更难排查。

如果项目通过 HBuilderX 运行 `uni-app` 或 `uni-app x`，还需要升级到 HBuilderX `>=5.11`，确保 IDE 内置 Node 能正确加载当前 ESM 依赖。
