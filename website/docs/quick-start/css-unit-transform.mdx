...
如果希望 `px` 按 `1:2` 转换成 `rpx`（例如 `10px` -> `20rpx`），可以传入下面配置：
...
在 `uni-app x` 等官方推荐使用 `px` 的场景，可以启用 `unitsToPx`，把 `rem/em/vw/vh/vmin/vmax/rpx` 等单位转换为 `px`。默认关闭。
...
> 注意：如果同时开启 `px2rpx` / `rem2rpx`，`unitsToPx` 会先转换成 `px`，后续还会继续转成 `rpx`。如果希望最终输出为 `px`，请不要同时开启 `px2rpx` / `rem2rpx`。
