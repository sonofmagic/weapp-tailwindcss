...
:::tip 新项目优先选择生成模式
如果你正在接入 Tailwind CSS 或准备升级到 v5，优先从本页开始。生成模式会直接生成小程序目标 CSS，并用 `classSet` 精确约束 JS / 模板转译范围。
:::
...
| --- | --- | --- | --- | --- |
| uni-app Vue Vite | `WeappTailwindcss` | `vite.config.ts` | `src/main.css` | 新项目优先选择，只注册 weapp-tailwindcss |
...
| Taro Vite | `WeappTailwindcss` | `config/index.ts` 的 `compiler.vitePlugins` | `src/app.css` | 不需要额外注入 Tailwind 官方 PostCSS 插件 |
