...
核心变化是：实际项目中不需要再注册 `@tailwindcss/vite`、`@tailwindcss/postcss` 或其他 Tailwind 官方构建插件；构建插件或 PostCSS 插件默认会接管样式生成，最终产物会直接面向小程序，而不是先生成浏览器 CSS 再做大量兜底兼容。
...
不传 `generator` 时会使用默认的 `mode: 'auto'` 和 `target: 'weapp'`。小程序选择器兼容配置也有默认值，`cssChildCombinatorReplaceValue` 默认就是 `['view', 'text']`，只有项目需要改成自定义组件标签时才需要传 `styleOptions`。
