...
Taro 项目根据编译器选择 Vite 或 Webpack 入口，同时需要先区分 Tailwind CSS 主版本。Tailwind CSS v3 和 v4 都会默认由 `weapp-tailwindcss` 接管样式生成。
...
Tailwind CSS v3 项目继续使用 `tailwind.config.js` 的 `content`，但不要再注册原来的 `tailwindcss` PostCSS 插件。CSS 入口由 `cssEntries` 指向项目全局样式。
