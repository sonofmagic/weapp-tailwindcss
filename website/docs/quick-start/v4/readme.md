...
常规 Vite 项目会在构建过程中自动识别被引入的 Tailwind CSS 入口。Webpack、Gulp、自定义构建、多入口或自动识别失败时，再显式配置 `cssEntries`，让 `weapp-tailwindcss` 和 `tailwindcss` 保持一致的处理模式。`cssEntries` 只负责补充识别，入口 CSS 仍然要被项目实际 import 或纳入构建图。
...
这种情况，你就非常需要兼容性降级方案，即使用 [`postcss-preset-env`](https://www.npmjs.com/package/postcss-preset-env)（`weapp-tailwindcss` 已经内置了这个插件，可通过 [`cssOptions.cssPresetEnv`](/docs/api/options/general#cssoptions) 配置）。
