...
- `@variant` / `@custom-variant` 管理状态类（如 `dark:` 与业务属性状态）

掌握这些指令后，就可以在不离开 CSS 语法的前提下扩展 Tailwind。

:::note 小程序交互状态
小程序不支持 CSS `:active`。`weapp-tailwindcss` 默认从小程序样式产物中删除 `active:*` 生成的 selector，但 H5 与 App 的 Web 构建仍会保留。小程序按压反馈应优先使用组件的 `hover-class`。
:::
...
- Tailwind 4 的编译沿用增量模式，项目自己的 watch 脚本生成的原子类会写入缓存。需要彻底清理时删除 `.tailwind`、`node_modules/.cache/tailwind`。
- 小程序开发者工具不支持 `@layer`，若遇到覆盖问题，可以开启 `cssOptions.cssPresetEnv` 的降级行为，详情见 [高阶篇](/docs/quick-start/v4/tutorial/advanced)。
