...
| 名称 | 说明 |
| --- | --- |
| Node.js `>=22.12.0` | `weapp-tailwindcss@5.2.0` 起的最低版本，默认支持从 CommonJS 加载 ESM |
...
通过 HBuilderX 运行 `uni-app` / `uni-app x` 时，还需要 HBuilderX `>=5.11`。

## 步骤二：注册 weapp-tailwindcss 插件

以 `weapp-vite` 为例（不同框架请对照对应的 [默认模式参考](/docs/tailwindcss/v4-reference) 和框架注册页）：
...
Tailwind CSS 4 项目应显式配置 `cssEntries`。它让 `WeappTailwindcss` 稳定读取入口 CSS 里的 `@import "tailwindcss"`、`@source`、`@config` 与运行时 class 来源，避免不同框架、不同平台的构建图差异导致样式缺失。
