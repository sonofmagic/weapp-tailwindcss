...
常规 Vite 项目会自动识别被引入的 Tailwind CSS 入口。只有多入口、自动识别失败或自定义构建链无法稳定发现入口等场景，才需要手动配置 `cssEntries`。`cssEntries` 不是替代 import 的开关，入口 CSS 仍然要通过框架的全局样式入口或业务入口文件被实际引入。
...
- 确认入口 CSS 是否被构建器引入；多入口或自动识别失败时，再检查 `cssEntries` 是否指向已经被项目引入的入口 CSS
