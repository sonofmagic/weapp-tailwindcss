...
module.exports = defineConfig({
  ...
  pluginOptions: {
    mpx: {
      plugin: {
        postcssInlineConfig: {
          // Mpx 例外：CSS 生成挂在框架 PostCSS 管线里，但这里注册的是 weapp-tailwindcss/postcss
...
> Mpx 是生成模式里的少数例外：CSS 生成放在 `postcssInlineConfig` 中，但注册的是 `weapp-tailwindcss/postcss`，不要再注册官方 `@tailwindcss/postcss`。Webpack 插件里的 `generator: false` 只避免 CSS generator 重复执行，模板和 JS 转译仍由 `WeappTailwindcss` 负责。
