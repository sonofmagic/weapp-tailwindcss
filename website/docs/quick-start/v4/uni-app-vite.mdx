...
> 常规 Vite 项目会自动识别被引入的 Tailwind CSS 入口。只有多入口、自动识别失败或自定义构建链无法稳定发现入口等场景，才需要手动配置 `cssEntries`。
> `cssEntries` 只负责补充识别，入口 CSS 仍然要通过 `App.vue`、`main.ts` 或框架全局样式入口实际引入到项目里。
