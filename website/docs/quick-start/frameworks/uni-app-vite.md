...
入口 CSS 还需要被项目实际引入，例如在 `src/App.vue` 的全局 `<style>` 中 `@import "./app.css";`，或通过框架推荐的全局样式入口导入。`cssEntries` 只负责让 `weapp-tailwindcss` 补充识别 Tailwind 入口，不会替代框架把这个 CSS 文件纳入构建图。
...
常规 Vite 项目会自动识别被引入的 Tailwind CSS 入口。多入口、自动识别失败或自定义构建链无法稳定发现入口时，再手动配置 `cssEntries`，但对应 CSS 文件仍然要被项目实际引入：
