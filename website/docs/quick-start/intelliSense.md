...
title: IDE 智能提示设置
description: tailwindcss-intellisense 在 v4 中必须看到 @import "tailwindcss" 才会将工作区视为 Tailwind 根文件。
keywords:
  - 快速开始
  - 安装
  - 配置
...
- 如果不想生成独立文件，可以直接在真实入口写 `@import 'tailwindcss';`，并把这个入口写入 `tailwindCSS.experimental.configFile`。

保存/重载任意文件后 VS Code 会检测到该辅助文件，从而让 Tailwind CSS 4 项目享受到完整的补全、悬浮和跳转体验。
