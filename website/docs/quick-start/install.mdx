...
生成模式下，Tailwind CSS v3 和 v4 都由 `weapp-tailwindcss` 接管样式生成。项目中仍然需要安装 `tailwindcss`，但不要再把 `tailwindcss` PostCSS 插件注册到实际项目里。

> 注意：各个框架的 PostCSS 注册位置很可能不同。比如 `uni-app vue3 vite` 项目必须使用 `vite.config.ts` 里的内联 `postcss` 选项，详见下方注意事项。
