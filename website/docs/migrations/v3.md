...
2. PostCSS 能力收敛到独立包 `@weapp-tailwindcss/postcss`
...
假如你已经升级到了 `pnpm@11.x`，安装依赖后需要执行 `pnpm approve-builds weapp-tailwindcss` 将其加入 `allowBuilds`，以便 `postinstall` 等 `npm hook` 能正常运行
