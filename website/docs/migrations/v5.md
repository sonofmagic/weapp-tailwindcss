...
6. H5/Web 构建不要再简单禁用 `WeappTailwindcss`。
...
v5 会根据常见环境变量自动切换目标。例如 `UNI_PLATFORM=h5`、`UNI_UTS_PLATFORM=h5/web/web-*`、`TARO_ENV=h5`、`MPX_CLI_MODE=web`、`MPX_CURRENT_TARGET_MODE=web` 会走 Web 目标，输出浏览器可用的 Tailwind CSS，而不是小程序转义后的选择器。
