...
Tailwind CSS 4 项目应总是写清 `cssEntries`，并使用项目根目录解析出的绝对路径。显式入口更稳定，也更容易排查跨框架、跨平台、分包和多入口问题。
...
uni-app x 项目应显式写 `cssEntries`，指向纯 CSS 入口。

HBuilderX 本地运行时请使用你自己的项目脚本，按微信小程序、Android 和 iOS 目标分别验证。iOS 模拟器需要完整 Xcode，具体工具链要求以 HBuilderX 和 Xcode 当前版本为准。

迁移验收应覆盖：真实平台产物、CSS 后缀、入口样式、平台变体、HMR/增量构建，以及模拟器或设备中的最终运行效果。
...
- [uni-app x 接入页](/docs/quick-start/frameworks/uni-app-x)
