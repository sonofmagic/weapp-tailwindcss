...
| `weapp-tailwindcss/webpack4` | webpack@4 适配入口（旧版 Taro/uni-app 项目） | `import { WeappTailwindcss } from 'weapp-tailwindcss/webpack4'`
| `weapp-tailwindcss/vite` | Vite 插件入口（Taro Vite、weapp-vite 等） | `import { WeappTailwindcss } from 'weapp-tailwindcss/vite'`
| `weapp-tailwindcss/postcss` | PostCSS 插件入口（Mpx、手写 PostCSS 配置等） | `const weappTailwindcss = require('weapp-tailwindcss/postcss')`
...
:::tip 插件命名
Vite、Webpack、Webpack4 与 Gulp 入口都导出 `WeappTailwindcss` 和 `weappTailwindcss` 两个别名。文档推荐使用大写 `WeappTailwindcss`；小写 `weappTailwindcss` 适合偏函数式命名的项目。PostCSS 入口继续使用默认导出。
:::
