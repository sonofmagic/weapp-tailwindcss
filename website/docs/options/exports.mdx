...
| `weapp-tailwindcss/vite/web` | Generic Vite Web 专用入口，固定 Web 目标并跳过小程序框架探测 | `import tailwindcss from 'weapp-tailwindcss/vite/web'`
...
标准 Vite Web 或 SSR 项目可以使用专用入口，无需配置 `appType`、`generator.target` 或 `tailwindcssBasedir`：
...
该入口只接管 Tailwind Web CSS 生成和必要的 HMR；小程序模板/JS 转译、分包样式和平台 CSS 适配不会进入 Web 生产热路径。需要关闭生成优化时可传入 `optimize: false` 或 `{ minify: false }`。
