...
当前修复为 Tailwind CSS 4 小程序 deferred/incremental 生成路径保留完整的变量声明及作用域。对构建期固定且可解析的变量显式配置 `cssOptions.cssCalc: ['--spacing']` 时，生成器在主题作用域改写前判断静态化资格，再计算最终长度：
...
这修复库侧的上下文传递、作用域判断和缓存失效，绕开微信对小 `rpx` 基数执行运行时乘法时的中间换算。同一输出存在局部选择器覆盖、条件规则或不同来源的冲突值时，相关变量及其依赖链保留运行时表达式；独立输出不共享变量值。主题或配置改变后，增量生成会重新计算受影响的规则。
...
- **预计算需要固定的变量上下文。** 当前构建链路保留 Tailwind v4 的有效主题和完整原始声明，在小程序主题选择器改写前计算固定变量。`--spacing: 1rpx` 配合 `cssCalc: ['--spacing']` 可以输出 `32rpx` 等最终长度。已知动态覆盖、来源冲突、无法解析或未选择的变量保留表达式，也不会仅因存在 `var()` fallback 就被冻结。
