...
通过提取和组合，可以把原子类封装成更稳定的组件样式。`daisyUI` 的思路也类似，只是它把这些样式预先整理成了一个 Tailwind CSS 插件。
...
`UnoCSS` 延续了 `Windi CSS` 的很多思路，在原子化 CSS 的即时生成、预设和 transformer 上走得更远。它的社区也很活跃，很多功能会先在这里被验证。

`Tailwindcss` 的路线更保守，但生态更大，已有项目和第三方组件更多。遇到常见问题时，通常也更容易找到现成的解决方案。

两者不是简单替代关系。新项目可以按团队偏好选择；已有 Tailwind CSS 项目则更适合先把扫描、组件封装和构建边界理顺。
