...
| 场景 | 命令 | 说明 |
| --- | --- | --- |
| 给 Tailwind CSS 打补丁、注入 `rpx` 支持 | `npx weapp-tw patch` | 旧 CSS 后处理链路运行一次即可；支持 `--cwd` 指向子包目录。生成模式项目不需要写入 `postinstall`。 |
...
| 查看补丁应用/缺失情况 | `npx weapp-tw status --json` | 复用 `tailwindcss-patch` 的状态检查，输出各补丁是否已应用，可配合 `--cwd` 指定子包。 |
...
:::tip 生成模式项目不需要配置 postinstall
如果项目已经使用 `weapp-tailwindcss@5` 生成模式，并且 Tailwind CSS 由 `WeappTailwindcss`、`weapp-tailwindcss/postcss` 或生成器入口接管，就不需要再在业务项目的 `package.json` 中配置 `postinstall: "weapp-tw patch"`。

`weapp-tw patch` 主要保留给旧 CSS 后处理链路、需要手动排查补丁状态的项目，或仍依赖 Tailwind patch 缓存的兼容场景。
:::
...
- 复用 `tailwindcss-patch` 的状态检查，按补丁列出 `applied`/`not-applied`/`skipped`/`unsupported`。
- `--cwd`：指定要检查的子包目录，默认沿用 `patch` 的自动推断。
- `--json`：输出 JSON 报告，方便 CI 或脚本读取。
