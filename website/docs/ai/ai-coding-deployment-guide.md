...
- **默认主力用国产（GLM/Qoder）**：低延迟、支付/报销友好、合规更容易。
- **海外模型做“白名单增强”**：仅给少数关键座席（架构/核心模块/疑难排查/代码审查），并且强制用量上限与审计。
- **Cursor 更像“体验型 IDE 平台”**：适合愿意统一 IDE 的团队；否则海外订阅分散到各 IDE/CLI 更灵活。

### 2.2 “必须满足”清单（不满足就别选海外主力）

- 公司已有 **稳定海外网络**（代理/VPN）并且可覆盖 IDE 与 CLI（同一套代理策略）。
- 安全合规确认：代码出境允许范围、日志留存、审计要求、敏感文件屏蔽策略已落地。
- 采购与支付可闭环：信用卡/虚拟卡、账单中心、成本中心分摊、发票/税务路径明确。
- 能做 **用量治理**：团队配额、个人上限、告警、超额降级策略。
...
| Qoder                        | 国产/混合路由        | ¥59-99（Pro/Pro+） | ~¥995（Team\*5）    | 国产为主        | 需要审计与团队管理、混合路由   |
...
| ---- | --------- | ------------ | ------------------------- |
| Pro  | ¥59       | ~500 次      | 普通席位                  |
| Pro+ | ¥99       | ~800 次      | 需要更稳质量/偶尔混合路由 |
| Team | ¥199/用户 | ~2,000 次/人 | 需要审计/团队治理         |
...
- 2 人：Cursor Pro（2 _ ¥144 = ¥288/月）或 Qoder Pro+（2 _ ¥99 = ¥198/月）
- 合计：约 ¥1,000/月（视组合而定）
...
- 80 人：GLM Pro
- 15 人：Qoder Team 或 GLM Max（根据是否需要审计/团队治理）
...
| 事项           | 技术总监 | 平台/中台 | 安全合规 | 采购财务 | 业务团队 TL |
| -------------- | -------- | --------- | -------- | -------- | ----------- |
| 方案选型与原则 | A        | C         | C        | C        | C           |
| 代理/网络方案  | C        | A/R       | C        | -        | C           |
| 仓库分级与红线 | C        | R         | A/R      | -        | C           |
...
1. 普通席位 `Pro`，重度/混合需求 `Pro+`，需要审计与成员管理用 `Team`。
