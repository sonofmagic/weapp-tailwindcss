...
- 样式方案从「全局命名」走向「模块化 / 组件化」再到「原子化」，本质上是在持续降低耦合、减少重复、提升可组合性。
- 原子化 CSS 的价值不只在于写得快，更在于它更适合和 tokens、JIT、组件变体、AI 生成这些现代工作流接起来。
- 但它也不是终点。多 runtime（Web、RSC、小程序）场景下，真正稳定的核心仍然是 token，而不是某一种具体写法。
...
- `Token + Headless`：最适合统一体验和 AI 协作；代价是必须认真维护 design system。
...
- 优势：组件 API 与样式解耦，变体集中管理，最适合统一设计语言。
...
- `Headless + tokens`：`Radix`、`Headless UI`、`shadcn/ui`、`tailwind-variants`
  常见用法：tokens -> variants -> primitives。
...
- `Token + Headless`：语义和信息密度都高，是更接近设计系统形态的方案。
