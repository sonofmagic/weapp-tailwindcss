...
多端框架项目建议在 H5/Web 与小程序构建里都保留 `WeappTailwindcss`：插件会按环境变量把 H5/Web 生成器目标切到 `web`，小程序目标继续输出小程序 CSS。两端可以共享同一个包含 `@import "tailwindcss"`、`@theme` 和 `@source` 的 CSS 文件，但不要在同一个多端构建里同时注册两套 Tailwind 生成插件。只有独立 Web 应用不经过小程序构建链路时，才适合继续单独使用 Tailwind 官方 Vite/PostCSS 插件。
