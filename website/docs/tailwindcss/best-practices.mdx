...
```tsx title="推荐：先对齐 token，再消费 token"
export function PromoCard() {
  return (
...
```ts title="不推荐：运行时拼接任意类"
const cls = `bg-${color}-${level} px-${size}`
```

```ts title="推荐：把动态空间收敛成枚举"
