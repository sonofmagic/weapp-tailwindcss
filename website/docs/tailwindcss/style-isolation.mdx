...
<TailwindTopicHero topicId="tailwindcss/style-isolation" title="样式隔离方案与原理" />
...
## 命名空间（Tailwind prefix）

- **原理**：通过 Tailwind CSS 4 的 import 参数给所有生成类名加前缀，减少与宿主类名冲突。
- **配置示例**：
...
- **注意**：仍是全局样式，宿主的 `!important` 仍可能干扰；preflight 仍会影响宿主（可关闭）。需要提高整套 utilities 的优先级时，可使用 `@import "tailwindcss" important;`。
...
- **做法**：只导入 theme 与 utilities，不导入 `preflight.css`，让宿主决定全局 reset。
