...
- 它先把设计系统拆成最小可复用单位，例如颜色、间距、圆角、字号；这些值会变成一组原子类。
- 然后在组件层重新把这些原子类组织起来，长成 `Button`、`Badge`、`Card` 这样的可读接口。
- 所以 Tailwind 真正解决的，不只是“少写 CSS”，而是让设计值、类名和组件 API 尽量指向同一份事实。

### 先记住这一句话

> Tailwind 不是为了让你到处堆类名，而是为了让设计 token 更容易被复用、约束和审阅。
...
- 先把设计值收敛成 token，再写类名。这一步看起来慢，实际上是在给后面所有组件减负。
- class 里最好引用 token，而不是直接写裸值。这样换品牌、换主题、做暗色模式时，改的是一层映射，不是全项目搜字符串。
- 团队要重点约束的不是“有没有用 Tailwind”，而是“是不是每个人都在随手发明新值”。

换句话说，入口阶段最重要的目标只有一个：让“设计稿里的值”和“代码里的值”能一一对上。
...
这一层解决的是“这些类怎么被生成出来，以及为什么有些项目会越用越重”。
