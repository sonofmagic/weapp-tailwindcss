...
<TailwindTopicHero topicId="tailwindcss/tailwind-core" title="Tailwind 设计理念" />
...
:::warning 微信小程序中的 rpx spacing
把全局 `--spacing` 设置为 `1rpx`、`3rpx` 等值时，生成的运行时乘法可能产生尺寸偏差。`@theme inline` 不会自动完成乘法；成功预计算成静态长度后，属性也不再响应对 `--spacing` 的运行时覆盖。详见 [微信小程序 --spacing 与 rpx 计算限制](../issues/spacing-rpx.md)。
:::
...
- **候选扫描 + `@source`**：Tailwind 只会生成它扫描到的类，所以 `@source` 写得越准，最终 CSS 越干净。很多包体问题，不是 Tailwind 本身慢，而是扫描范围太宽或者动态类太随意。
