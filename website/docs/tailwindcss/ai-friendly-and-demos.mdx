...
```mermaid
flowchart LR
  Prompt[结构化提示模板] --> Gen[模型生成 React/Vue 代码]
  Gen --> Merge[cn(twMerge) 去重]
  Merge --> Lint[ESLint + Tailwind IntelliSense]
  Lint --> Build[pnpm build / dev]
  Build --> Review[人工 Spot Check]
  Review --> Snapshot[截图/Story 基线]
  Snapshot --> Deploy[上线/分发]
  Build --> Metrics[CSS 体积/coverage]
  Metrics --> Review
```
