...
<TailwindTopicHero topicId="tailwindcss/merge-and-variants" title="tailwind-merge、cva、tailwind-variants 精要" />
...
- 需要让组件接受 `className` 覆盖时，先把 `tailwind-merge` 接好。
- 单槽组件优先用 `cva`，多槽组件再考虑 `tailwind-variants`。
- 不要把 variants 散落在业务组件里，应该集中在 builder 文件里。

## 常见误解

- 不要把 `cva` 或 `tailwind-variants` 误解成 Tailwind 官方自带能力。它们是社区里非常常见、但并非唯一的组件变体组织方式。
- 不要把 `tailwind-merge` 理解成“可有可无的小优化”。只要组件允许外部传 `className` 覆盖，它通常就是稳定性的基础设施。
- 不要把“用了 builder”理解成“从此不需要评审类名”。builder 只是在集中表达变体，不会自动替你建立设计约束。

## tailwind-merge：去重与边界

- 可以把它理解成一个“Tailwind 类名裁判”。
- 它会根据 Tailwind 的冲突规则，决定哪些类应该保留、哪些类应该被删掉。
- 这样你才能真正做到“调用方最后传入的意图生效”，而不是把结果交给 Tailwind 内部排序碰运气。
- 最常见的用法就是封装 `cn = twMerge(clsx(...))`，然后把它作为所有 class 拼接的统一入口。
...
如果你在手机上只扫一眼，这一节的核心其实就是一句话：没有 `tailwind-merge`，组件开放 `className` 往往就不够可靠。
...
- `cva`：更轻，更适合按钮、徽标、输入框这类单槽组件。
- `tailwind-variants`：更完整，更适合卡片、弹窗、菜单这类多槽组件。
