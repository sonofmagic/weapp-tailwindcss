...
- **自动匹配 Tailwind CSS 主版本**：包内同时内置 `tailwind-merge@2`（对应 `tailwindcss@3`）与 `tailwind-merge@3`（对应 `tailwindcss@4`），安装时会根据项目里的 `tailwindcss` 版本自动切换入口。
...
安装脚本会尝试解析项目根目录里 `tailwindcss` 的版本，并把 `dist/index.*` 指向对应的运行时：

- `tailwindcss@^4` ➜ 使用 `@weapp-tailwindcss/merge/v4`（基于 `tailwind-merge@3`）
- 其他情况 ➜ 使用 `@weapp-tailwindcss/merge/v3`（基于 `tailwind-merge@2`）

若检测失败，会回退到 `v4` 版本并打印提示。你也可以通过环境变量显式控制：

| 环境变量 | 作用 |
| --- | --- |
| `WEAPP_TW_MERGE_TARGET_VERSION` / `WEAPP_TW_MERGE_FORCE_VERSION` | 强制运行时版本（支持 `3`/`v3`/`4`/`v4`） |
| `WEAPP_TW_MERGE_DISABLE_FALLBACK` / `WEAPP_TW_MERGE_NO_FALLBACK` | 检测失败时不要覆盖入口，手动按需引入 `v3`/`v4` |
| `WEAPP_TW_MERGE_STRICT` | 切换失败时让脚本返回非零退出码 |
| `WEAPP_TW_MERGE_RESOLVE_ROOT` | 指定解析 `tailwindcss` 的额外搜索目录 |
...
`@weapp-tailwindcss/merge` 也封装了 [`tailwind-variants`](https://tailwind-variants.org/)，在小程序环境中同样可以使用 `tv`/`cn` 输出带转义的类名：
...
更多组合用法、`cn` 聚合器以及逃逸策略见后文的 [`cva 与 tailwind-variants 支持`](./cva-and-variants) 章节。
