...
`packages-runtime` 是 `weapp-tailwindcss` 的运行时能力集合，用来把编译期的类名规则延伸到小程序端，解决「动态类名冲突」「非法字符转义」「多端一致性」等问题。它们基于 `@weapp-tailwindcss/runtime` 提供 escape/unescape 与工厂能力，当前文档面向 Tailwind CSS 4。
...
| `@weapp-tailwindcss/cva` | v4 | 组件变体工厂 | `class-variance-authority` 运行时封装 |
