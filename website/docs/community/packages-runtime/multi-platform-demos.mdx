...
本页示例统一以 **Tailwind v4** 为主线（`@weapp-tailwindcss/merge` + `@weapp-tailwindcss/variants`）。当前文档仅维护 Tailwind CSS 4 接入说明。
...
- 多端项目建议统一使用 Tailwind CSS 4 入口，并保持构建插件启用；这里只按端切换 runtime 的 escape/unescape。
