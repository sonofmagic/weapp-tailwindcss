...
为什么? 因为之前有一个问题，`tailwind-merge` / `class-variance-authority` / `tailwind-variants` 没有什么很好的办法在小程序里使用。

因为 `weapp-tailwindcss` 在编译时，就已经把 `wxml` / `js` / `wxss` 中提取到的 tailwindcss 字符串 token 给转译了
...
编写 `tailwind-merge` 插件，或者 create 一个新的 `tailwind-merge`

失败了

## 思路二

也就是之前的方案，在编译时忽略 `twMerge` / `twJoin` / `cva` 等函数内部的转义，然后在第一次运行时，进来的时候，是完整的 tw 类名

于是我编写了 `@weapp-tailwindcss/merge` 的 v1 版本。

看上去有用，但是实际操作的时候出现了大问题，因为这种方式是依赖于 ast 节点的解析的，但是我编写的插件，是无法保证运行时得到的类名是完整的，中间经历的步骤非常有可能会把，函数名，或者模板字符串的标识符名称给压缩重命名了
