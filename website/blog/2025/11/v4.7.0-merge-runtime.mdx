...
最近我一直在思考 weapp-tailwindcss 的未来，以至于玩国服回归的星际争霸2都不香了。
...
摆在我面前的，是一条看不到未来的路：为了兼容，我需要重写它的核心，fork 一个全新的包，这个成本是巨大的。
...
另外表面上看这条路是可行的，我们甚至找到了几个 demo 可以跑通，我还抽离出了 `ignoreCallExpressionIdentifiers` 配置项，以为自己解决了问题。
...
所以成功之后，写了这篇文章，算是给 `weapp-tailwindcss` 的未来定调：我们终于和旧版 `@weapp-tailwindcss/merge` 告别，把全部逃逸逻辑搬到了运行时。也顺手解决了“编译期豁免 + 手动逃逸”的历史包袱。

## 为什么要重写 merge？

复盘 1.x 旧版 merge，我发现我们当时的设计基于两个假设：一是 tailwind-merge 的输入输出始终可控，二是编译器可以精准标记所有“需要放行”的调用。随着生态摸索出更多组合式 API，这两个假设已经被现实击碎。
...
在实现里我刻意把 escape 和 unescape 拆成两个“齿轮”。不管是用户直接手点 `twMerge`，还是 variants 工厂兜一圈回来，都会先进统一的预处理，再丢给 tailwind-merge 啃。这等于在运行时补了一层“语义编译器”。
...
问题马上就来了：它完全做不到配对 `unescape`。`[` 和 `]` 被一起砸成 `_`，`(` / `)`、`{` / `}` 也全堆在同一个值上，运行时根本还原不回去。举个让人头疼的例子：`escape('[bg:red]') === '__bg_red_'`，你说首尾两个 `_` 到底是 `[` 还是 `]`？想指望它在运行时“先解码再 merge”，纯属自找麻烦。
...
另外还开放了 `map` 字段，队友如果想统一用自己的字符映射，照配即可，类名风格也能和现有规范保持一致。
...
既然逃逸已经搬回运行时，我脑海里的下一步就是：把那些黑名单、注入脚本全删了吧。不过要这么干，得把它的原罪摊开讲清楚——黑名单到底帮不上哪些忙？

`ignoreCallExpressionIdentifiers` 生下来就是补丁王，只能勉强做到：

- 给某些函数贴个“别转义”标签，但压根保证不了运行时会再逃逸；
- 遇到链式调用、解构、动态导入立刻迷路；
- **压缩器一上来就把它摧毁**。交给 `terser` / `esbuild` / `rolldown` 后，函数名会被改成 `e()`、`r()`，AST 对不上名，除非你一遍遍去配置 `mangle.reserved` 告诉它“拜托别动 twMerge”，维护成本炸裂；
- 与各种插件之间要保持“边际合作”，Babel/SWC/JITI 都得手写一份实现，谁来维护？

等我们开始支持 variants、运行时工厂、Tailwind v4 更多奇葩写法之后，这黑名单就彻底变成误导。每次 code review 看到 `ignoreCallExpressionIdentifiers`，我脑子里冒出来的都是“为啥要替它开绿灯”，而不是“这样真的安全吗”。难怪用户会问：

> “编译时不是放我过去了吗，怎么页面照样报 `invalid selector`？”

升级到运行时方案以后，这些疑问直接消失。所有入口的 escape 都在 merge 里兜底，任何调用都只会吐出合法类名，编译阶段终于可以回归“安静做事”的状态。

## 4.7.0 版本的实际成果

绕了这么多弯，所有成果最终都塞进了 4.7.0。想法 -> PoC -> 实战，我们来回跑了三轮，才敢拍板放出来。交付物一股脑列在这里：

- `resolveTransformers` 负责统一调度 escape/unescape，还能 toggle、换 mapping，想怎么玩都行；
- `twMerge` / `twJoin` / `createTailwindMerge` / `extendTailwindMerge` / `cva` / `variants` 全家桶共用同一套转换逻辑，再也没有“谁忘了接管”的 bug；
- 单测把 tailwind-merge 的功能列表扫了一遍，还额外加了小程序专属场景，例如 `rpx`、任意变体、强制修饰符等；
- `apps/vite-native-ts` Demo 直接把不同版本的 merge 行为、运行时选项、CVA/variants 输出实时展示，谁来都能一眼看懂；
- 写了中文 Changeset，清清楚楚告诉大家：这是一次 breaking change，升级请打起精神。

## 展望

把逃逸逻辑拉回运行时之后，roadmap 上那些问号终于能擦掉了。我们不用再在“性能”和“灵活”之间拔河，而是多了一条可持续迭代的中线。目前这些能力已经落袋：

- **跨端一致**：[weapp-vite](https://github.com/weapp-vite/weapp-vite)、uni-app、甚至你自建的打包链，只要套 runtime 工厂就能保证输出安全；
- **DSL 更放得开**：variants 案例已经证明，我们可以放心封装更高级的组合 API，类名合规这件事交给 merge；
- **插件可以减负**：Babel/SWC 回到“老实做替换”的岗位，黑名单补丁统统退休；
- **未来更好扩展**：下一步准备把 mapping 插件化，按平台或团队切换 escape 字典，想怎么拼就怎么拼。

4.7.0 算是 weapp-tailwindcss 运行时时代的第一声号角。欢迎大家把新版 `@weapp-tailwindcss/merge` 用到真实项目里，更欢迎在社区继续砸想法，我会把这些反馈当作下一轮迭代的燃料，让 Tailwind CSS 在小程序世界里始终“开箱即用”。
