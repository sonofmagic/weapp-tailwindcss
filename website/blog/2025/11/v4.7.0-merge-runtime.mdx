...
为什么? 因为之前有一个问题，`tailwind-merge` / `class-variance-authority` / `tailwind-variants` 没有什么很好的办法在小程序里使用。它们代表着 tailwind 的“运行时表达力”，而我们多年来积累的方案几乎全部寄托在编译阶段。

因为 `weapp-tailwindcss` 在编译时，就已经把 `wxml` / `js` / `wxss` 中提取到的 tailwindcss 字符串 token 给转译了，我们以“提前落盘、提前逃逸”来确保上线安全。

而 `tailwind-merge` / `class-variance-authority` / `tailwind-variants` 这一些工具的合并操作，都在运行时进行。

完全兼容不了啊!

当社区开始越来越多地讨论“class-variance-authority 要怎么落地”时，我意识到问题已经不是“某个工具不支持”那么简单，而是整个生态在呼唤新的联动方式：编译期的确定性和运行时的灵活性必须重新组合。

## 思路一

最直观的念头，是不是只要为 `tailwind-merge` 写一个 weapp-tailwindcss 专用插件就好了？我把 tailwind-merge 的内部冲突表导出，尝试用自定义 escape hook 覆盖那些非法字符；甚至写了一个半成品的 `createTailwindMerge` 变体，希望能在编译阶段就生成完全符合小程序命名规则的类名。

可现实很快给了我当头棒喝：`tailwind-merge` 对运行时字符串的依赖极强，我们要么重写它的核心，要么就得 fork 一个全新的包。那段时间我对比了几个版本，也向社区里的朋友请教过“在 merge 之前做 escape 是否可行”，得到的答案都指向同一个结论——当工具的设计哲学不同（静态 vs 动态），单靠插件无法把它们揉成一个方案。

结束那周的实验记录时，我在 README 旁边写下了两个字：失败。

## 思路二

第二条路看起来更务实：沿用我们熟悉的编译期管线，给 `twMerge` / `twJoin` / `cva` 等函数做“豁免处理”。只要在编译时忽略它们内部的转义，运行时拿到的就是完整的 class 字符串，那 tailwind-merge 不就能工作了吗？

于是我编写了 `@weapp-tailwindcss/merge` 的 v1 版本。

表面上看这条路是可行的，我们甚至找到了几个 demo 可以跑通，还在内部文档里记录了“如何声明 ignoreCallExpressionIdentifiers”的操作指引。然而一旦项目规模变大，问题立刻浮现：这套方案高度依赖 AST 解析和构建工具的配合，我写的插件无法保证运行时得到的类名永远完整。构建链路上的任一环节——Terser、esbuild、rollup 插件甚至手写 Babel 宏——都可能把函数名或模板字符串的标识符压缩重命名，最后留给运行时的是一个残缺的字符串。

那一刻我意识到，编译期黑名单只是在延迟爆炸时间，而不是解除危机。

在两条路都走到尽头之后，只剩下一个选择：彻底重构 merge，让逃逸逻辑回归运行时，让编译阶段恢复简单纯粹。

这篇文章算是给 `weapp-tailwindcss` 的未来定调：我们终于和旧版 `@weapp-tailwindcss/merge` 告别，把全部逃逸逻辑搬到了运行时。也顺手解决了“编译期黑名单 + 手动逃逸”的历史包袱。
