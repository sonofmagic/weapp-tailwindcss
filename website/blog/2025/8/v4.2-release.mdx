...
### 1. 安装并引入 Tailwind CSS

这篇发布日志中的 Tailwind CSS 3 接入示例已经迁移到旧版文档。当前站点只保留 Tailwind CSS 4 接入说明；如果项目必须继续使用 `tailwindcss@3`，请安装 `weapp-tailwindcss@4` 并查看 [v4 文档站](https://v4.tw.icebreaker.top/)。
...
当前接入方式已经合并到 [快速使用](https://tw.icebreaker.top/docs/quick-start/install)。不再需要配置 `postinstall: "weapp-tw patch"`。

### 3. uni-app x 中注册 weapp-tailwindcss

#### 3.0. 配置 Tailwind CSS 入口
    ...
    plugins: [
        ...
        WeappTailwindcss(
            uniAppX({
                base: __dirname,
                cssEntries: [
                    new URL('./main.css', import.meta.url).pathname,
                ],
                rem2rpx: true,
            }),
        ),
...
当前 Tailwind CSS 4 配置请查看 [uni-app x 接入文档](/docs/quick-start/frameworks/uni-app-x)。旧版 Tailwind CSS 3 配置请查看 [v4 文档站](https://v4.tw.icebreaker.top/)。
