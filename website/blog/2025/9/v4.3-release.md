...
> 后续版本已将 CSS 变量计算模式调整为默认关闭，以避免大体积自定义属性在兼容处理后重复展开；需要该兼容能力时请显式配置 `cssOptions.cssCalc: true`。

# weapp-tailwindcss@4.3.0 🚀

## CSS变量计算模式

在 `tailwindcss@4` 下，默认启用 CSS 变量计算模式。
...
默认模式会保留原始 `calc()` 声明，方便继续遵循标准 CSS 级联。如果目标小程序运行时会采用后面的 `calc()`，导致前面的预计算兜底值被覆盖，需要显式指定要清理的 CSS 变量。

> 可通过给插件，传入 `cssCalc` 配置项 `false` 来手动关闭这个功能

假如这时候你需要去除匹配 `--spacing` 的原始 `calc()` 声明，可以传入
...
详见: https://tw.weapp.dev/docs/api/interfaces/UserDefinedOptions#csscalc
