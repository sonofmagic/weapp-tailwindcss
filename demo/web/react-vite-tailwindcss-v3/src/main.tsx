    ...
    <main className="min-h-screen bg-zinc-50 p-6 text-zinc-900 dark:bg-zinc-900 dark:text-zinc-50">
      <section className="mx-auto grid max-w-4xl gap-6">
        <div className="rounded border border-zinc-900/10 bg-white p-5 shadow-[0_8rpx_24rpx_rgba(0,0,0,0.12)] dark:bg-zinc-900">
          <h1 className="text-[32rpx] font-semibold">React Vite Tailwind CSS v3</h1>
          <p className="mt-3 text-sm leading-6 text-zinc-600 dark:text-zinc-300">
            Web target is the default. Use build:weapp to compare mini-program CSS output.
          </p>
          <div className="theme-mode-demo mt-4 rounded bg-white px-4 py-3 text-slate-900 system-dark:bg-slate-900 system-dark:text-slate-100 theme-dark:bg-zinc-900 theme-dark:text-zinc-50 dark:bg-zinc-900 dark:text-zinc-50">
            React Vite Tailwind CSS v3 system dark
            <div className="theme-dark mt-2 rounded bg-white px-3 py-2 text-slate-900 theme-dark:bg-zinc-950 theme-dark:text-zinc-50">
              React Vite Tailwind CSS v3 manual dark
            </div>
          </div>
        </div>
        <div className="grid grid-cols-2 gap-[16rpx] md:grid-cols-4">
          {rows.map(item => (
            <div key={item} className={`${item} rounded p-[24rpx] text-center before:mr-1 before:content-['v3']`}>
              swatch
            </div>
          ))}
        </div>
